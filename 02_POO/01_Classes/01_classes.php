...
<?php
    class User {
        ...
        public function test() {
            echo "La vie est dur, signé " . $this->nom;
        }
        
        public function returnSalaire() {
            return $this->salaire;
        }
    }

    $user = new User('Alice', 30, 20000); // Instanciation de classe

    var_dump($user->nom);
    var_dump($user->age);
    // ça fonctionne pas parce que la propriété est privé et n'est qu'accessible QUE dans la classe
    // var_dump($user->salaire);
    echo $user->nom . "<br>";

    // Pour acceder à la propriété salaire on utilise un getter (une méthode pour récupérer unbe propriété privé)
    echo $user->returnSalaire();
?>
