...
<body>
    ...
    <code>
        <?php
            ...
            //On vas vérifier si les informations transmises via le formulaire sont existantes ou non
            $nom = isset($_POST["nom"]) && !empty($_POST["nom"]) ? $_POST["nom"] : "Le nom n'est pas fourni";

            // Même choose que le ternaire du dessus
            ...
            $prenom = isset($_POST["prenom"]) && !empty($_POST["prenom"]) ? $_POST["prenom"] : "Le prénom n'est pas fourni";
            ...
            $hacked = $isset($_GET['prenom']) ? echo "je rajoute du code malveillant pour récupérer des infos : echo "je laisse tomber je poursuit la malveillance mouahahahaha!!"

            Avec la fonction, les instructions ne seront pas interprétées mais écrite telles qu'on les voit ;)

            À utiliser quand on affiche des données et pas quand on les envoit !
            */

            // 2- DATA GET METHOD

            $keyName = $nom = isset($_GET["keyName"]) && !empty($_GET["keyName"]) ? $_GET["keyName"] : "Le nom n'est pas fourni";
            $augmen = $nom = isset($_GET["motif"]) && !empty($_GET["motif"]) ? $_GET["motif"] : "Le motif n'est pas fourni";
            echo "Nom (via GET): La personne la plus malveillante du PoleS est : " . htmlspecialchars($keyName). "<br>" ;
            echo "Motif (via GET): sous le motif de : " . htmlspecialchars($augmen) . "<br>" ;
        ?>
    </code>
